    ...
    <>
      ...
      <div className="mt-8 text-center md:hidden">
        <p className="text-[13px] text-slate-500 font-medium">
          Already have an account? <Link href={`/login?redirectTo=${redirectTo}`} className="text-blue-600 font-bold hover:text-blue-700">Sign In</Link>
        </p>
      </div>
    </>
  );
...
function LoginLink() {
  ...
  return (
    <Link
      ...
      className="px-8 py-3.5 rounded-full border border-slate-300 text-slate-700 bg-white hover:bg-slate-50 hover:border-slate-400 font-heading font-black tracking-widest text-xs uppercase transition-all shadow-sm w-full max-w-[200px] text-center"
    >login
...
// 2. The Main Interactive Layout Export
export default function RegisterClient() {
  return (
    <main className="min-h-[100svh] flex items-center justify-center bg-slate-50 font-sans p-6 relative overflow-hidden">
      ...
      <div className="absolute bottom-[-10%] left-[-5%] w-[600px] h-[600px] bg-cyan-100/50 blur-[120px] rounded-full pointer-events-none" />

      {/* Main Glass Card */}
      <div className="w-full max-w-4xl flex flex-col md:flex-row-reverse bg-white border border-slate-200 rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.05)] overflow-hidden relative z-10">

        {/* Right Side Visually (Brand Panel) */}
        <div className="w-full md:w-1/2 bg-slate-50 p-10 md:p-14 flex flex-col items-center justify-center text-center relative border-b md:border-b-0 md:border-l border-slate-200 overflow-hidden">
        ...
        {/* Left Side Visually (Form Panel) */}
        <div className="w-full md:w-1/2 p-10 md:p-12 flex flex-col justify-center bg-white">
          <div className="mb-8 text-center md:text-left">
            <h1 className="text-3xl font-heading font-black text-slate-900 mb-2 tracking-tighter uppercase">Create Account</h1>
            <p className="text-sm font-medium text-slate-500">Apply for clearance to continue</p>
          </div>
