...
export default function DodoXClient() {
  ...
  return (
    <div className="pb-24">
      ...
      {/* ── HERO — robot LEFT, text RIGHT ── */}
      <section className="relative min-h-screen flex items-center overflow-hidden bg-[#0A0A0A]">
        {/* Subtle glow only — no grid */}
        <motion.div
        ...
        <motion.div
          ...
          className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-accent-blue/10 rounded-full blur-[120px] pointer-events-none"
        />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 w-full pt-32 pb-16">
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4 }}>
            <Link href="/"
              className="inline-flex items-center gap-2 text-gray-500 hover:text-white text-sm font-bold transition-colors duration-200 mb-16 group"
            >
              <span className="w-8 h-8 rounded-full bg-white/5 border border-white/10 flex items-center justify-center group-hover:bg-accent-blue group-hover:border-accent-blue transition-all duration-200">
          ...
          {/* Hero split — robot LEFT, content RIGHT */}
          <div className="flex flex-col lg:flex-row gap-12 items-center">

            {/* LEFT — Robot image (like home page flip card) */}
            <motion.div
            ...
            >
              <div className="relative rounded-3xl overflow-hidden bg-gradient-to-br from-[#0d1520] to-[#080808] border border-white/10 min-h-[500px] flex items-center justify-center">
                {/* Corner brackets */}
            ...
            >
              ...
              <h2 className="font-black text-lg md:text-xl text-gray-400 uppercase tracking-widest mb-6">
              ...
              <motion.div
                ...
                className="origin-left w-16 h-0.5 bg-accent-blue rounded-full mb-8"
              />
              ...
              <div className="flex flex-wrap gap-2">
                {["ROS2 Humble", "Raspberry Pi 5", "OAK-D", "Nav2", "SLAM"].map((tag) => (
                  <span key={tag} className="bg-white/5 border border-white/10 text-gray-400 text-xs font-bold px-3 py-1.5 rounded-full">
                    {tag}
                  </span>
                ))}
              </div>
      ...
      <section className="max-w-7xl mx-auto px-6 mt-24">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
          ...
          <h2 className="font-black text-3xl text-white uppercase tracking-tighter mb-10">
            Every <span className="text-accent-blue">Angle</span>
          </h2>
        </motion.div>
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex lg:flex-col gap-3 lg:w-[150px] order-2 lg:order-1">
            {ANGLES.map((a, i) => (
              <button key={i} onClick={() => setActiveAngle(i)}
                className={`relative rounded-xl overflow-hidden border flex-1 lg:flex-none lg:h-[110px] transition-all duration-200 ${
                  activeAngle === i ? "border-accent-blue shadow-lg shadow-accent-blue/20" : "border-white/10 hover:border-white/25"
                }`}
              >
                <img src={a.src} alt={a.label} className="w-full h-full object-contain bg-[#0d1117] p-2" />
          ...
          <div className="flex-1 order-1 lg:order-2">
            <AnimatePresence mode="wait">
              <motion.div key={activeAngle}
                initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }}
                transition={{ duration: 0.3 }}
                className="relative rounded-2xl overflow-hidden bg-gradient-to-br from-[#0d1520] to-[#080808] border border-white/10 min-h-[480px] flex items-center justify-center"
              >
                <img src={ANGLES[activeAngle].src} alt={ANGLES[activeAngle].label}
                  className="relative z-10 max-h-[420px] object-contain"
                />
                {/* Nav arrows */}
                <button onClick={() => setActiveAngle((p) => (p - 1 + ANGLES.length) % ANGLES.length)}
                  className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/50 border border-white/10 flex items-center justify-center hover:bg-accent-blue hover:border-accent-blue transition-all duration-200 z-20"
                >
                  <ChevronLeft className="w-5 h-5 text-white" />
                </button>
                <button onClick={() => setActiveAngle((p) => (p + 1) % ANGLES.length)}
                  className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/50 border border-white/10 flex items-center justify-center hover:bg-accent-blue hover:border-accent-blue transition-all duration-200 z-20"
                >
                  <ChevronRight className="w-5 h-5 text-white" />
                </button>
                {/* Label */}
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-black/70 backdrop-blur-sm border border-white/10 rounded-full px-4 py-1.5 text-xs font-bold text-white z-20">
      ...
      <section className="max-w-7xl mx-auto px-6 mt-24">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
          ...
          <h2 className="font-black text-3xl text-white uppercase tracking-tighter mb-8">
            Under the <span className="text-accent-blue">Hood</span>
          </h2>
        </motion.div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {SPECS.map((s, i) => (
            <motion.div key={s.label}
              ...
              className="bg-[#111111] border border-white/5 rounded-xl p-5 hover:border-accent-blue/30 transition-colors duration-300 group"
            >
              <div className="font-mono text-[10px] uppercase tracking-widest text-gray-600 mb-2">{s.label}</div>
      ...
      <section className="max-w-7xl mx-auto px-6 mt-24">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
          ...
          <h2 className="font-black text-3xl text-white uppercase tracking-tighter mb-8">
            What It <span className="text-accent-blue">Does</span>
          </h2>
        </motion.div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {FEATURES.map((f, i) => (
            <motion.div key={f.title}
              ...
              className="group bg-[#111111] border border-white/5 rounded-xl p-6 hover:border-accent-blue/30 hover:shadow-lg hover:shadow-accent-blue/5 transition-all duration-300"
            >
      ...
      <section className="max-w-7xl mx-auto px-6 mt-24">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
          ...
          <h2 className="font-black text-3xl text-white uppercase tracking-tighter mb-10">
            Development <span className="text-accent-blue">Timeline</span>
          </h2>
        </motion.div>
      ...
      <section className="max-w-7xl mx-auto px-6 mt-24">
        <motion.div
          ...
          className="relative bg-[#111111] border border-white/10 rounded-3xl p-12 flex flex-col md:flex-row items-center justify-between gap-8 overflow-hidden"
        >
